...
def grouped_bar(metric, ylabel, title, fname, labels, bl_rows, pl_rows, ylim=None):
...
def main():
    ...
    if not os.path.isfile(csv_path):
        print(f"ERROR: {csv_path} not found. Run build_final_table.py first.", file=sys.stderr)
        return 1
    ...
    grouped_bar("EO_gap", "Equal Opportunity Gap",
                ...
                "eo_gap_comparison.png", labels, bl_rows, pl_rows, ylim=(0, 1.05))

    # --- Figure 2: Accuracy ---
    grouped_bar("Accuracy", "Accuracy",
                "Accuracy: Baseline vs Fairness-Aware Pipeline",
                "accuracy_comparison.png", labels, bl_rows, pl_rows, ylim=(0, 1.15))
