...
`FairSynData/` carries no licence, so its source is **not redistributed here** — neither the
original files nor modified copies of them. Files below that are this project's own work are
included and can be copied straight in; the three that are *modifications to existing
FairSynData source* are specified as instructions in **[`CHANGES.md`](CHANGES.md)** instead.

Paths are relative to a `FairSynData/` checkout root.
...
| *(see `CHANGES.md` §2)* | `mydatasets/DatasetFactory.py` | Registers `Adult`/`Credit` alongside `Law`/`Dutch` |
| *(see `CHANGES.md` §3)* | `mycodes/datasetsPreprocess.py` | Adds `adult`/`credit` branches to `preprocess_data`/`scale_data`/`split_data_to_tensors` (same pattern as the existing `law`/`dutch` branches), plus a non-numeric-column guard in `detect_outliers` (diagnostic-only function, its output is logged, never used to filter data) |
| *(see `CHANGES.md` §1)* | `mycodes/myParams.py` | `AlgorithmParams.K`: `3` → `20` (outer bilevel iterations — the smoke-test default was too small to let the fairness-constrained synthetic data actually converge; see `docs/COMPARISON.md` for why this wasn't actually the main blocker) |
...
2. Copy this folder's own files into your `FairSynData/` checkout at the listed paths, and
   apply the three source modifications from [`CHANGES.md`](CHANGES.md).
