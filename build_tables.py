...
# German is a secondary dataset (scripts/run_german.sh, not scripts/run_final.sh) but
# uses the same draft_results_<ds>_final_seed<N>.json naming, so it slots into T1/T2
# the same way. It has no sweep/dirichlet/ablation runs, so it's intentionally left out
# of T3/T4/T5 below rather than showing fabricated "FAILED" rows for data that was never
# collected.
WINNER_SINGLE = {
    ...
    "german": "draft_results_german_final_seed1.json",
}
WINNER_5SEED = {
    ...
    "german": [f"draft_results_german_final_seed{s}.json" for s in range(1, 6)],
}
...
def main():
    ...
    doc = ["# Paper Tables — Credit Risk, UCI Adult, Law School & German Credit", "",
    ...
    doc.append(md_table(
        ...
        "Credit/Law: winner config (`dp=none, tune_threshold=true`, intercept on). "
        "Adult/German: winner rho/eps with `tune_threshold=false`, intercept off, to "
        "report raw accuracy (German is secondary — see docs/PROJECT_STATUS.md).",
        *t1))
    doc.append(md_table(
