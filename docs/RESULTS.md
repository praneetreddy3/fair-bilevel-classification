...
- Figures for the manuscript: `outputs/final_pareto.png`, `outputs/final_bars.png` (one point
  ...
  `outputs/results_pareto.png`, `results_bars.png`, `results_convergence.png` (from
  `python plot_results.py`) are the full dev/diagnostic sweep — every logged config, useful for
  debugging but too dense for the paper.
