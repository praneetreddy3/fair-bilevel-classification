...
**For the manuscript, use these two** (generated by `python plot_final_results.py`, one point
per dataset = mean ± std over the 5 final seeds — clean and readable):

- **`outputs/final_pareto.png`** — accuracy vs. EO gap, one baseline (star) + one pipeline
  (circle) point per dataset, error bars = std across seeds.
- **`outputs/final_bars.png`** — baseline vs. pipeline accuracy and EO gap, grouped bars with
  error bars, one group per dataset.

The figures below are the full **dev/diagnostic** sweep (every config ever logged in
`outputs/` — final runs, sensitivity sweep, ablations, dirichlet, per-seed winners; 50+ points).
Useful while debugging, too dense to put in the paper:
...
- **`outputs/results_pareto.png`** (generated by `python plot_results.py`) — accuracy vs. EO gap
  across every logged single-seed config, baseline marked with a star per dataset; broad but
  noisy full-sweep view, uses the `run_final.sh` configs rather than the validation-selected ones.
- **`outputs/results_bars.png`** — same full sweep as a grouped bar chart — quick visual for "did
  the fairness intervention help or hurt, and by how much," at the cost of readability once
  there are many logged runs.
